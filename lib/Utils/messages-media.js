...
const getWAUploadToServer = ({ customUploadHosts, fetchAgent, logger, options }, refreshMediaConn) => {
    return async (stream, { mediaType, fileEncSha256B64, newsletter, timeoutMs }) => {
        ...
        // ============ KEY FIX: REORDER HOSTS TO PRIORITIZE mmg.whatsapp.net ============
        // Get the default hosts from uploadInfo
        const defaultHosts = [...uploadInfo.hosts];
        
        // Create custom priority list with mmg.whatsapp.net first
        // First try custom hosts if provided
        // Then try mmg.whatsapp.net specifically
        // Then other hosts from uploadInfo
        const priorityHosts = [];
        
        // 1. Add custom upload hosts first (if any)
        if (customUploadHosts && customUploadHosts.length > 0) {
            priorityHosts.push(...customUploadHosts.map(host => ({ hostname: host, maxContentLengthBytes: undefined })));
        }
        
        // 2. Always try mmg.whatsapp.net FIRST - this is the key fix
        priorityHosts.push({ hostname: 'mmg.whatsapp.net', maxContentLengthBytes: undefined });
        
        // 3. Then add other default hosts (excluding mmg if already in list)
        for (const host of defaultHosts) {
            if (host.hostname !== 'mmg.whatsapp.net') {
                priorityHosts.push(host);
            }
        }
        
        // Use our priority hosts instead of the default order
        const hosts = priorityHosts;
        // ============ END OF FIX ============
        ...
        // Log the prioritized hosts for debugging
        logger?.debug(`Upload hosts priority: ${hosts.map(h => h.hostname).join(', ')}`);
        ...
        for (const { hostname, maxContentLengthBytes } of hosts) {
            ...
            try {
                ...
                if ((result === null || result === void 0 ? void 0 : result.url) || (result === null || result === void 0 ? void 0 : result.directPath)) {
                    ...
                    // Log successful host
                    logger?.debug(`✅ Successfully uploaded to: ${hostname}`);
                    break;
                }
                else {
